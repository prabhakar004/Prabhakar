...
spark.conf.set("spark.sql.shuffle.partitions","120")#40 #APLD
...
def load_files_parallel(group_conf=[]):
    ...
    for conf in group_conf:
        ...
        try:
            ...
            logger.info("total records available in the file {0}".format(file_name))
            ...
            if transform_df.count() > 0:
                ...
                logger.info("Stitching DF count is '{0}' for the table '{1}'".format(count_var,curated_tbl_name))
                if count_var > 0:
                    stitching_list_full = []
            ...
            returnDict.append({'file_id': file_id, 'conf_id': conf_id, 'publish_count': pub_rec_cnt, 'status': status,
                    's3_path': s3_path_pub, 'file_name': file_name, 'message': message})
        except Exception as e:
            ...
            returnDict.append({'file_id': file_id, 'conf_id': conf_id, 'publish_count': 0, 'status': 'FAILED', 's3_path': '',
                    'file_name': file_name, 'message': str(e.args)})
    return returnDict
...
def load_stitch_del_flag(conf={}):
    ...
    try:
        if target_object_name in catalog_tables:
            ...
            df_output_table_name.createOrReplaceTempView(source_temp_view)
            if target_object_name.lower() not in ['pub_ptnt_actvty_fct_cdl_history','pub_ptnt_mdcr_prt_d_fct_cdl_history']:
...
if __name__ == "__main__":
    source_system = 'shs_apld_wkly_history'
    secret = cdl_lib.get_secret_manager('gmb_cdl_postgres')
    stitching_data_query = """(select min(batch_id) from cdl_ingestion_log where data_source='{source_system}' and published_status !='SUCCESS')query_wrap""".format(source_system=source_system)
    print("stitching_data_query",stitching_data_query)
    min_batch_df = cdl_lib.read_from_db(secret=secret,tbl_query=stitching_data_query)
    print("min_batch_df :",min_batch_df.collect())
    batch_id = min_batch_df.collect()[0][0]
    print("batch_id",batch_id)
    
    workflow_params={"curated_crawler":"cdl_curated_crawler",
    "curated_db":"cdl_curated_db",
    "environment":"prod",
    "fm_address_email":"gara@genmab.com",
    "new_parentlog":batch_id,
    "no_of_thread":"5",
    "parent_batch_process_name":"SYMPHONY_APLD_BATCH_PROCESS",
    "publish_crawler":"cdl_publish_crawler",
    "publish_db":"cdl_publish_db",
    "rds_secret_id":"gmb_cdl_postgres",
    "s3_cdl_bucket":"gmb-cdl-prod-us",
    "s3_cdl_curated_bucket":"gmb-cdl-prod-curated-us",
    "s3_cdl_publish_bucket":"gmb-cdl-prod-publish-us",
    "s3_vendor_bucket":"gmb-us",
    "source_system":source_system,
    "to_address_email":"msap@genmab.com,santhoshkumar.gopal@agilisium.com"}
    ...
    for pr_col in range(min_priority,max_priority+1):
        ...
        final_status_result.extend(status_result)
